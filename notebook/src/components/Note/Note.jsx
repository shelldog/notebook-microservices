...
const behaveState = (behave) => {
  if (behave === 'off') return { content: 'planned', style: 'status-off', next: 'on' };
  if (behave === 'on') return { content: 'progress', style: 'status-on', next: 'finished'};
  if (behave === 'finished') return { content: 'finished', style: 'status-finished'}
}

export default function Note({ children, note }) {
  ...
  return (
    <div className='m-l note-c'>
      ...
      <div className='note-b flex jc-sb'>
        {children}
        <button className={`note-button ${status.style}`} onClick={onSubmit}>
          {status.content} 
        </button>
      </div>
    </div>
